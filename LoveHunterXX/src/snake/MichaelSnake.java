...
public class MichaelSnake extends MovingComponent {
	...
	public void addPresent(DavidPresent p) { // adding body parts.
		...
		p.setY(presentList.get(presentList.size() - 1).getY()); // finish from
																// here
	...
	public void moveCoors(Direction d) {
		if (presentList == null || gameRunning == false)
			return;
	...
	public boolean checkLose() {
		for (int i = 1; i < presentList.size(); i++) {
			if (cart.isCollided(presentList.get(i))) {
				setRate(999999);
				gameRunning = false;
				// System.out.println("Game Over. You ran into yourself. You
				// have earned " + presentList.size()/10 + " LovePoints.");
				LoveHunterXX.ss.updateText(
						"               Game Over.                   You ran into yourself. You have            earned "
								+ presentList.size() / 10 + " LovePoints.");
				nextPart();
				return true;
			}
		}
		...
		if (cart.getX() < 30 || cart.getX() > 415 || cart.getY() < 50 || cart.getY() > 440) {
			setRate(999999);
			gameRunning = false;
			// System.out.println("Game Over. You ran into a wall. You have
			// earned " + presentList.size()/10 + " LovePoints.");
			LoveHunterXX.ss.updateText(
					"                Game Over.                    You ran into a wall. You have              earned "
							+ presentList.size() / 10 + " LovePoints.");
			nextPart();
			return true;
		}
	...
	public boolean checkWin() {
		if (presentList.size() > 30) {
			setRate(999999);
			gameRunning = false;
			// System.out.println("You win! You have collected enough presents
			// to please your girlfriend! LovePoints +3!");
			LoveHunterXX.ss.updateText(
					"                 You win!                          You have collected enough               presents to please your                          girlfriend!                                  LovePoints +3!");
			nextPart();
			return true;
		}
		return false;
		...
		for (int i = 0; i < MichaelDavidSnakeScreen.gens.size(); ++i) {
			...
			if (cart.isCollided((MichaelInteractable) MichaelDavidSnakeScreen.gens.get(i))) {
				...
				if (collided.getName() == "LoveGift") {
					...
					// SnakeScreen.gens.get(SnakeScreen.gens.size() -
					// 1).generateNew(SnakeScreen.gens);
